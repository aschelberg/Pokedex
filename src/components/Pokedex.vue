...
const fetchBookmarks = (name) => {
  const bookmarks = JSON.parse(localStorage.getItem("bookmarkedPokemon"));
  const foundBookmark = bookmarks.find((t) => t.name === name)
  return foundBookmark ? foundBookmark.bookmarked : false
}
  ...
  <div class="bg-white">
    ...
    <div>
        ...
        <Dialog class="relative z-40 lg:hidden" @close="mobileFiltersOpen = false">
          ...
          <div class="fixed inset-0 z-40 flex">
            <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="translate-x-full">
              <DialogPanel class="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-6 shadow-lg">
      ...
      <main class="mx-auto px-1 lg:max-w-7xl lg:px-8">
        ...
        <div class="pb-12 pt-4 sm:grid sm:grid-cols-3 md:grid-cols-5 md:gap-x-2 lg:grid-cols-6">
          <aside>
            <h2 class="sr-only">Filters</h2>
            
            <button type="button" class="inline-flex items-center md:hidden px-2" @click="mobileFiltersOpen = true">
          ...
          <section aria-labelledby="product-heading" class="mt-4 sm:col-span-3 md:col-span-4 md:mt-0 lg:col-span-5">
            <h2 id="product-heading" class="sr-only">Pokemon</h2>

            <div class="pl-2 col-span-5">
              <!-- Pagination -->
              <div class="flex items-center justify-between bg-white pb-3" aria-label="Pagination">
              ...
              <ul role="list" class="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                <li v-for="pokemon in allPokemon" class="border border-blue-600 col-span-1 flex flex-col rounded-lg bg-white shadow">
                  <PokemonCard :pokemon="pokemon" :bookmarked="fetchBookmarks(pokemon.name)"/>
                </li>
              </ul>
            </div>
          </section>
      ...
      <footer aria-labelledby="footer-heading" class="border-t border-gray-200 bg-white">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="border-t border-gray-100 py-10 text-center">
            <p class="text-sm text-gray-500">&copy; 2024 Your Company, Inc. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
